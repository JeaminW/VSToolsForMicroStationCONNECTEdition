...
namespace VSTMC
{
    ...
    internal sealed class OpenMdlAppsFolderCommand
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="OpenMdlAppsFolderCommand"/> class.
        ...
        {
            // Switch to the main thread - the call to AddCommand in OpenMdlAppsFolderCommand's constructor requires
            // the UI thread.
            await ThreadHelper.JoinableTaskFactory.SwitchToMainThreadAsync(package.DisposalToken);

            OleMenuCommandService commandService = await package.GetServiceAsync((typeof(IMenuCommandService))) as OleMenuCommandService;
            Instance = new OpenMdlAppsFolderCommand(package, commandService);
        }
        ...
        private void Execute(object sender, EventArgs e)
        {
            ...
            try
            {
                if (Directory.Exists(utilities.MSCEMDLPath))
                {
                    utilities.OpenFolderLocation(utilities.MSCEMDLPath);
                }
            }
            catch (Exception)
            {
